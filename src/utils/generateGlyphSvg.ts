...
function readFont(filePath: string): Buffer | null {
...
let gothicFont = readFont('C:\\Windows\\Fonts\\seguihis.ttf')
if (!gothicFont) {
    console.warn('Segoe UI Historic font not found. Trying fallback noto sans gothic font...')
    gothicFont = readFont(
        join(
            process.cwd(),
...
export async function generateGlyphSvg(char: string, script: string): Promise<string> {
    ...
    const fontFamily = isGothic ? 'Segoe UI Historic' : 'Noto Sans'
    ...
    if (gothicFont) {
        fonts.push({ name: 'Segoe UI Historic', data: gothicFont, weight: 400, style: 'normal' })
    }
